...
describe('WebRTCService', () => {
  ...
  describe('handleOffer', () => {
    ...
    it('should process pending ICE candidates after setting remote description', async () => {
      ...
      webRTCService.pendingCandidates = [{ candidate: 'candidate1' }, { candidate: 'candidate2' }];
    ...
    it('should handle errors in setRemoteDescription', async () => {
      ...
      await expect(webRTCService.handleOffer({ type: 'offer', sdp: 'test-sdp' })).rejects.toThrow(
        error,
      );
    });
  });
  ...
  describe('handleAnswer', () => {
    it('should throw an error if peer connection is not initialized', async () => {
      await expect(webRTCService.handleAnswer({ type: 'answer', sdp: 'test-sdp' })).rejects.toThrow(
  ...
  describe('waitForConnection', () => {
    ...
    it('should reject if connection fails', async () => {
      ...
      const handleFailure = vi
        .spyOn(webRTCService, 'emit')
        .mock.calls.find((call) => call[0] === 'connectionStateChange')?.[1];
  ...
  describe('waitForRemoteOffer', () => {
    ...
    it('should wait for remoteOfferReceived event', async () => {
      ...
      const onSpy = vi.spyOn(webRTCService, 'on');
      const offSpy = vi.spyOn(webRTCService, 'off');
      const offerPromise = webRTCService.waitForRemoteOffer();

      // Should have registered event handler
      expect(onSpy).toHaveBeenCalledWith('remoteOfferReceived', expect.any(Function));
      ...
      expect(offSpy).toHaveBeenCalledWith('remoteOfferReceived', expect.any(Function));
    });
  ...
  describe('connection state event handlers', () => {
    ...
    it('should handle connection state failed', () => {
      ...
      const attemptRecoverySpy = vi
        .spyOn(webRTCService as any, 'attemptRecovery')
    ...
    it('should handle ice candidate event with candidate', () => {
      ...
      expect(emitSpy).toHaveBeenCalledWith('iceCandidate', event.candidate);
    });
    ...
    it('should handle ice candidate event without candidate', () => {
      ...
      expect(emitSpy).not.toHaveBeenCalledWith('iceCandidate', expect.anything());
    });
